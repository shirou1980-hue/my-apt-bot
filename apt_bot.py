...
def fetch_via_public_api(today: datetime) -> list | None:
    ...
    try:
        ...
        # API 자체 에러 처리 (인증 실패 등)
        if header_obj.get("resultCode") != "00":
            print(f"[API] 서버 응답 오류: {header_obj.get('resultMsg')}")
            return None

        # 🔥 [치명적 맹점 해결] 데이터가 없을 때 items가 문자열 ""로 오는 현상 완벽 방어
        items_data = body_obj.get("items", {})
        if not items_data or isinstance(items_data, str):
        ...
        print(f"[API] 예측하지 못한 치명적 오류 발생: {e}")
        return None
